...
-- end AKK_ACCOUNT
-- begin AKK_OPERATION
alter table AKK_OPERATION add constraint FK_AKK_OPERATION_ACC1 foreign key (ACC1_ID) references AKK_ACCOUNT(ID)^
alter table AKK_OPERATION add constraint FK_AKK_OPERATION_ACC2 foreign key (ACC2_ID) references AKK_ACCOUNT(ID)^
alter table AKK_OPERATION add constraint FK_AKK_OPERATION_CATEGORY foreign key (CATEGORY_ID) references AKK_CATEGORY(ID)^
...
-- end AKK_OPERATION
-- begin AKK_BALANCE
alter table AKK_BALANCE add constraint FK_AKK_BALANCE_ACCOUNT foreign key (ACCOUNT_ID) references AKK_ACCOUNT(ID)^
